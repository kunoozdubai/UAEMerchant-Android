...
public class ResponseHandler {
	 ...
	 */
	public static void purchaseResponse(final Context context, final PurchaseState purchaseState, final String productId, final String orderId,
			final long purchaseTime, final String developerPayload) {
		Log.i("InApp", "BillingService() - Response Handle - purchaseResponse");

		if (purchaseState == PurchaseState.PURCHASED) {
			if(CommonConstants.AD != null && CommonConstants.POST_LISTENER != null){
				new AddPicturesDialog(context, CommonConstants.AD, CommonConstants.POST_LISTENER).show();
			}
		...
		new Thread(new Runnable() {
			public void run() {
				...
				//
				// InAppRecord inAppRecord = new InAppRecord(orderId, productId,
				// "PURCHASED",
				// String.valueOf(purchaseTime), developerPayload);
				// new
				// InAppRecordDBHandler(MainSkyMenu.getGameContext()).insertNewInAppRecord(inAppRecord);
				//
				// //making Api call on purchase success.
				// int itemId= atmItem.getItemId();
				// int payType= DreamLifeGameConstants.PayType.GOOGLE;
				// float payAmount=0;
				// if(atmItem.getSalePrice()==0){
				// payAmount= atmItem.getActualPrice();
				// }else{
				// payAmount= atmItem.getSalePrice();
				// }
				// new PurchasesApi().buyCashItemApiCall(itemId, payType,
				// payAmount, orderId);
				// //////////////////////

				// PurchaseDatabase db = new PurchaseDatabase(context);
				// int quantity = db.updatePurchase(
				// orderId, productId, purchaseState, purchaseTime,
				// developerPayload);
				// db.close();
				//
				...
				// purchaseState, productId, quantity, purchaseTime,
				// developerPayload);
				// }
				// }
			}
	...
	private static void showDialogOnTransactionCompletion(String msg) {
		Log.i("InApp", "showDialogOnTransactionCompletion");
		AlertDialog.Builder builder = new AlertDialog.Builder(context);
		builder.setTitle(InAppBillingConstants.TRANSACTION_RESPONSE);
